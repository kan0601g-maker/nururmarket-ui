...
const LS_KEY = "ahatouch_chirarizumu_images_meta_v1";
...
const STORE = "chirarizumu_images";
...
// ===== API =====
export const addChirarizumuImages = async (
  files: FileList | File[]
): Promise<StoredImage[]> => {
  if (typeof window === "undefined") return [];
  ...
  const current = listChirarizumuImages();
...
export const getChirarizumuImageSrcById = async (
  id: string
): Promise<string | null> => {
  if (!id) return null;
