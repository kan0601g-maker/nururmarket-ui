...
import { getChiraSrcById } from "../../_components/chirarizumuImages";
...
function Inner() {
  ...
  const diff: Difficulty =
    diffParam === "easy" || diffParam === "hard" || diffParam === "aha" ? diffParam : "normal";
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const s = await getChiraSrcById(id);
      if (!alive) return;
      setSrc(s);
    })();
    return () => {
    ...
    <main className="min-h-screen bg-black text-white">
      <div className="mx-auto max-w-6xl px-6 py-10">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold">CHIRARIZUMU PLAY</h1>
        ...
        {id && (
          <div className="mt-4 flex flex-wrap gap-2">
            {(["easy", "normal", "hard", "aha"] as Difficulty[]).map((d) => {
              const active = d === diff;
              return (
                <Link
                  key={d}
                  href={`/ahatouch/chirarizumu/play?id=${encodeURIComponent(id)}&diff=${d}`}
                  className={[
